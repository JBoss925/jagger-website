...
export const geneticTsPaper: PaperDocument = {
  ...
  description:
    "A technical paper for an interactive genetic algorithm demo that evolves projectile launch velocities under configurable physics.",
  ...
  previewCaption:
    "GeneticTS simulation preview. A population of launch velocities is evaluated through a physics scene and evolved toward a moving target.",
  actionLinks: [
    {
      label: "Open Demo",
      href: "/genetic-ts",
  ...
  sections: [
    {
      ...
      title: "Motivation",
      paragraphs: [
        "The project turns a genetic algorithm into something visible. Instead of optimizing an abstract score in a console, every candidate is a launch trajectory. The reader can watch the best path, see ghost attempts, drag the target, and change the physics while the population adapts.",
        "That makes it useful as both a demo and a debugging surface. Selection pressure, mutation rate, population size, and environmental difficulty are usually hidden inside algorithm logs. Here, each setting changes the shape of the paths on screen."
      ],
      bullets: [
      ...
      equations: [
        {
          ...
          tex: "g = (v_x, v_y)",
          caption:
            "A candidate solution is only the initial x and y velocity applied to the ball."
        },
        {
          label: "Miss score",
          tex: "F_{\\mathrm{miss}} = \\frac{5000}{1 + d_{\\min}} + 0.0025\\min(L, 900)",
          caption:
            "Unsuccessful attempts are scored by closest approach plus a small path-length tie breaker."
        },
        {
          label: "Hit score",
          tex: "F_{\\mathrm{hit}} = 10000 + 18(T - t_{\\mathrm{hit}})",
          caption:
            "Successful attempts dominate misses, and faster hits rank above slower hits."
        }
      ]
    },
    {
      ...
      equations: [
        {
          ...
          caption:
            "Child velocity is a weighted blend of two parent launch velocities."
        },
        {
          label: "Mutation",
          tex: "g'_c = g_c + (\\epsilon_x, \\epsilon_y)",
          caption:
            "When mutation fires, velocity jitter is scaled by the current wind and gravity settings."
        }
      ],
      bullets: [
    ...
    },
    {
      ...
      paragraphs: [
        "The implementation keeps the genetic algorithm in a pure simulation module and the visual playback in the React page. That separation matters because the evolution loop can be tested independently from the animated presentation.",
        "A deterministic Mulberry32 random source is used per seed and generation. That makes a generation reproducible without removing the user's ability to reroll targets or change the environment."
      ],
      bullets: [
    ...
    },
    {
      ...
      paragraphs: [
        "The demo works because the optimization target is understandable. A user does not need to know genetic algorithms to see the population improve from bad trajectories to accurate launches.",
        "The broader lesson is that algorithm demos benefit from direct manipulation. When the reader can drag the target or change the physics and see the population recover, the algorithm becomes less like a black box and more like a system with observable pressure."
      ],
      bullets: [
