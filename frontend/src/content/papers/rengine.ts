  ...
  sections: [
    {
      ...
      title: "Motivation",
      paragraphs: [
        "Rengine exists to explore the parts of a game or rendering engine that are usually hidden below the demo: how entities are represented, how transforms compose through parents, how rendering can be swapped, and how update logic stays separate from drawing.",
        "The project deliberately keeps the visual vocabulary small. Boxes, folders, paths, rotations, and debug markers are enough to stress the runtime boundaries without burying the engine ideas under art assets."
      ],
      bullets: [
      ...
      equations: [
        {
          ...
          caption:
            "Each entity carries position, anchor, rotation, and scale."
        },
        {
          label: "Composed rotation",
          tex: "r_{\\mathrm{world}} = (r_{\\mathrm{parent}} + r_{\\mathrm{local}}) \\bmod 2\\pi",
          caption:
            "Folder entities compose child rotation with parent rotation."
        }
      ],
      bullets: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "The update model is component-oriented. Components receive delta time, the current entity, the scene, and previous engine state, then return an updated entity and state. That keeps movement and animation logic attached to entities without putting it inside the renderer."
      ],
      equations: [
        {
          ...
          caption:
            "A component advances an entity and may also return a changed engine state."
        }
      ],
      bullets: [
    ...
    },
    {
      ...
      paragraphs: [
        "The portfolio integration upgrades the engine sandbox into a reader-facing demo. It keeps the live canvas, adds demo selection, zoom controls, wireframe toggling, scene-focus copy, and a collapsible runtime tree.",
        "That tree is the important bridge between visual output and engine state. A reader can watch the scene move, then inspect the corresponding hierarchy, component labels, local/world transforms, anchors, rotations, scales, and node IDs."
      ],
      bullets: [
    ...
    },
    {
      ...
      paragraphs: [
        "Rengine is valuable because it treats engine concepts as the product. The demo does not need complex gameplay to show whether transform composition, component updates, scene activation, and renderer boundaries make sense.",
        "The project also shows why inspectors matter. A visual bug in a scene graph is much easier to reason about when the tree, local transforms, world transforms, and debug markers are visible at the same time."
      ],
      bullets: [
