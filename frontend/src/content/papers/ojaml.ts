...
export const ojamlPaper: PaperDocument = {
            ...
            blocks: [
                ...
                },
                {
                    ...
                    items: [
                        ...
                        "The current scope is finite: top-level and nested modules, local type declarations, and value signatures are supported, but file imports, full signature type items, functors, exceptions, and garbage collection are not."
                    ]
                }
            ],
        },
  ...
  println "Hello, OJaml!"`,
                ...
                },
                {
                    ...
                    tex: "P ::= D^{*}\\quad\\quad D ::= \\texttt{let}\\;\\texttt{rec?}\\;x\\;q^{*}(\\texttt{:}\\tau)?=e\\mid\\texttt{module type}\\;S=\\texttt{sig}\\;V^{*}\\;\\texttt{end}\\mid\\texttt{module}\\;M(\\texttt{:}S)?=\\texttt{struct}\\;D^{*}\\;\\texttt{end}\\mid\\texttt{type}\\;...\\mid\\texttt{open}\\;M",
                    caption: "A program is a list of declarations; modules group values, types, and nested modules under qualified names, module types specify required values, and open declarations expose namespace members by short name."
                },
                {
                    kind: "paragraph",
  ...
  PListNil | PListCons | PWildcard | PVar`,
        ...
        },
        {
            ...
            blocks: [
                {
                    kind: "paragraph",
                    text: "OJaml uses a Hindley-Milner-style constraint system. Types are primitives, type variables, applications for tuples/records/arrays/lists/sets/maps, nominal variants with type arguments, and function types. Record type declarations add named record shapes to the checker environment; algebraic data type declarations add nominal variant type constructors and constructor bindings. Module-local types and constructors are stored under qualified names; declarations inside the same module receive short-name aliases for sibling and enclosing module types, and open declarations add short-name aliases for immediate exported values, types, and constructors. Value, function parameter, and higher-order function annotations resolve those names and unify the annotated position with the declared shape. Module signature ascription resolves each val type in the ascribed module's local type namespace and unifies it with the exported member's inferred type. Checking walks the AST, creates fresh type variables where information is not known yet, and unifies constraints as expressions demand relationships between values."
                },
                {
                    kind: "paragraph",
  ...
  Map.get m "one"`,
                ...
                },
                {
                    ...
                    items: [
                        ...
                        "Module tests cover parsing, qualified member calls, user-defined opens, module type declarations, val signature ascription, opened module-local types and constructors, local shadowing, sibling references, closures that capture module sibling values, duplicate diagnostics, unknown opens, missing signature members, mismatched signature types, and nested-module behavior.",
...
tests/*.test.ts     positive runtime tests and negative checker tests`,
                ...
                },
                {
                    kind: "bullets",
                    items: [
                        "Extending modules further requires file imports, full signature type items, functors, and richer namespace export rules beyond the current compile-time module surface.",
