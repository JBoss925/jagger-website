    ...
    sections: [
            ...
            blocks: [
                ...
                },
                {
                    ...
                    items: [
                        ...
                        "The current scope is finite: top-level and nested modules with local type declarations are supported, but file imports, module signatures, exceptions, and garbage collection are not."
                    ]
                }
            ],
        },
        {
            ...
            blocks: [
                {
                    kind: "paragraph",
                    text: "An OJaml program is a sequence of top-level let declarations, module declarations, top-level open declarations, record type declarations, and algebraic data type declarations with optional type parameters. A let declaration may be recursive, may bind parameters, may annotate those parameters, may carry a value annotation, and may be separated by optional double semicolons. Module declarations contain value bindings, nested modules, record types, and algebraic data types; module members are reached through qualified names such as Scores.total, Scores.Offsets.make, Geometry.point, or Geometry.Named, and open Geometry exposes immediate values, types, and constructors by short name when no local, top-level, or competing opened namespace owns that name. Expressions include primitives, tuples, tuple projection, records, field access, variables, unary and binary operations, sequencing, forward pipelines, conditionals, local lets, local function bindings, local recursive function bindings, function application, anonymous functions, constructor application, and match expressions."
                },
                {
                    kind: "example",
                    label: "Hello world",
...
let main =
  ...
  String.length (head words) + List.length words`,
                    caption: "Opened stdlib namespaces provide short names, while exact local or top-level names win and ambiguous names still require a qualified form."
                },
                {
                    kind: "example",
...
let main =
  ...
  if x > 100 then x else 0`,
                ...
                },
                {
                    ...
                    tex: "d ::= \\texttt{type}\\;x=\\{x:\\tau;\\ldots\\}\\mid \\texttt{let}\\;\\texttt{rec?}\\;x\\;x^{*}\\;(:\\tau)?=e\\\\ e ::= n\\mid f\\mid s\\mid b\\mid ()\\mid (e, e^{+})\\mid \\{x=e;\\ldots\\}\\mid e.x\\mid e.k\\mid x\\mid e\\;e^{+}\\mid \\texttt{fun}\\;x^{+}\\rightarrow e\\mid \\texttt{let}\\;\\texttt{rec?}\\;x\\;x^{*}\\;(:\\tau)?=e\\;\\texttt{in}\\;e\\mid \\texttt{if}\\;e\\;\\texttt{then}\\;e\\;\\texttt{else}\\;e\\mid \\texttt{match}\\;e\\;\\texttt{with}\\;(p\\rightarrow e)^{+}",
  ...
  PListNil | PListCons | PWildcard | PVar`,
        ...
        },
        {
            ...
            blocks: [
                {
                    kind: "paragraph",
                    text: "OJaml uses a Hindley-Milner-style constraint system. Types are primitives, type variables, applications for tuples/records/arrays/lists/sets/maps, nominal variants with type arguments, and function types. Record type declarations add named record shapes to the checker environment; algebraic data type declarations add nominal variant type constructors and constructor bindings. Module-local types and constructors are stored under qualified names; declarations inside the same module receive short-name aliases for sibling and enclosing module types, and open declarations add short-name aliases for immediate exported values, types, and constructors. Value and function parameter annotations resolve those names and unify the annotated position with the declared shape. Checking walks the AST, creates fresh type variables where information is not known yet, and unifies constraints as expressions demand relationships between values."
                },
                {
                    kind: "paragraph",
...
(func $main (result i32)
  ...
  | head :: tail -> head + sum tail`,
                ...
                {
                    kind: "example",
                    label: "Set and map destructuring",
                    language: "ocaml",
                    code: `let main =
  ...
  | _ -> 0`,
                    caption: "Set and map patterns use {| ... |}. They match the stored linked-entry order, which is newest insertion first for the current Set.add and Map.set helpers."
                },
                {
                    kind: "example",
  ...
  Map.get m "one"`,
                ...
                },
                {
                    ...
                    items: [
                        ...
                        "Module tests cover parsing, qualified member calls, user-defined opens, opened module-local types and constructors, local shadowing, sibling references, closures that capture module sibling values, duplicate diagnostics, unknown opens, and nested-module behavior.",
...
tests/*.test.ts     positive runtime tests and negative checker tests`,
                ...
                {
                    kind: "paragraph",
                    text: "The second cross-file invariant is representation agreement. The checker distinguishes int, float, bool, string, unit, tuples, records, arrays, lists, sets, maps, and functions. The emitter erases those distinctions to i32 only after type checking. Runtime helpers then interpret the i32 according to the static type that selected the helper."
                },
                {
                    kind: "bullets",
                    items: [
                        "Extending modules further requires file imports, signatures, functors, and richer namespace export rules beyond the current compile-time module surface.",
