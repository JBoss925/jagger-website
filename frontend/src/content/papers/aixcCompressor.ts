...
export const aixcCompressorPaper: PaperDocument = {
  ...
  previewCaption:
    "AIXC reference-codec preview. The project stores a seed, predictor hit/miss decisions, residual streams, and manifest-backed metadata in a deterministic archive.",
  sections: [
    {
      id: "motivation",
      eyebrow: "I",
      title: "Motivation",
      paragraphs: [
        "AIXC started from a compact idea: if both sides can run the same predictor, the archive should not have to store every next unit. It can store enough seed material to initialize context, then a decision stream saying whether the predictor was correct. On misses, the encoder stores the actual residual unit so decoding remains exact.",
        "That idea is attractive because it turns compression into an explicit contract between a file and a predictor. The hard part is not packing bits. The hard part is making sure a future decoder can reproduce the same prediction sequence, tokenizer behavior, and residual interpretation without guessing."
      ],
      bullets: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "A crucial implementation detail is that both encoder and decoder feed the actual decoded unit back into the predictor state. They do not let an incorrect prediction become the next context item. That keeps one miss from cascading into a permanently divergent decode."
      ],
      equations: [
        {
          ...
          caption:
            "Each coded position stores whether the deterministic top prediction matched the actual unit."
        },
        {
          label: "Decoder reconstruction",
          tex: "\\hat{u}_i = d_i\\,\\operatorname*{argmax}_u p(u \\mid \\hat{u}_{<i}) + (1-d_i)\\,r_i",
          caption:
            "A hit uses the predictor output; a miss consumes the next residual literal."
        }
      ]
    },
    {
      ...
      equations: [
        {
          ...
          caption:
            "With miss rate m, the reference byte codec pays one decision bit plus one raw byte for every miss."
        },
        {
          label: "Decision entropy",
          tex: "H_2(p) = -p\\log_2(p) - (1-p)\\log_2(1-p)",
          caption:
            "Clustered or biased hit/miss streams should code closer to binary entropy than to one full bit per unit."
        }
      ],
      bullets: [
      ...
      equations: [
        {
          ...
          caption:
            "A canonical predictor manifest can be hashed and recorded in the archive so decoders can reject mismatched runtimes."
        },
        {
          label: "Round-trip condition",
          tex: "\\operatorname{decode}(\\operatorname{encode}(x)) = x",
          caption:
            "Model-token mode is only exact when tokenizer encode/decode preserves the original text."
        }
      ],
      bullets: [
    ...
    },
    {
      ...
      paragraphs: [
        "The compressor repo is structured as a reference codec plus experiments around practical predictor choices. The dependency-free toy-byte backend is useful for tests and demos, while the LZP byte backend provides fast deterministic full-file runs. Additional modes explore trained byte models, neural byte predictors, Hugging Face causal language models, and native acceleration through a C extension.",
        "The project also includes a local web workbench. It can manage corpora, run compression jobs, inspect archive metadata, train byte or neural sidecar models, and regenerate benchmark reports. That makes AIXC less like a single codec script and more like a small laboratory for testing predictor contracts."
      ],
      bullets: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "AIXC becomes more interesting when the predictor is shared out of band. With corpus-specific max-context byte models, the order-64 archive-only ratios beat the best native archive ratio on all five selected logs: Linux reaches 0.012, Zookeeper 0.018, Apache 0.020, Mac 0.022, and Proxifier 0.022. Higher orders can drive some archive-only ratios below 0.001, but the model bytes dominate if each archive has to carry its own model.",
        "That makes the project useful in a specific class of systems: repeated, related text streams where the predictor can be installed once and amortized across many archives. Log families, telemetry exports, generated reports, protocol traces, or corpora distributed with a known sidecar model fit the design better than one-off files. For arbitrary standalone compression, a mature native codec is still the practical choice."
      ],
      bullets: [
