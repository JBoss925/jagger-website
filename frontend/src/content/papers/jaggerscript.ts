...
export const jaggerscriptPaper: PaperDocument = {
  ...
  date: "2026",
  abstract:
    "JaggerScript is a small typed scripting language implemented in TypeScript. The project includes a PEG grammar, a compiler pass that converts parser output into a typed intermediate program representation, an interpreter with stack frames and heap-backed object instances, and a browser playground powered by Monaco. The language supports classes, constructors, functions, primitive values, arithmetic and boolean expressions, conditional blocks, loops, break, return, instantiation, scoped member access, and console output examples. This paper explains the implementation as a language-tooling project: a deliberate separation between parsing, normalization, runtime evaluation, and the interface used to explore the language.",
  description:
  ...
  previewCaption:
    "JaggerScript playground. The browser surface combines editable source, built-in examples, syntax highlighting, diagnostics, and interpreter output.",
  actionLinks: [
    {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "The type vocabulary is intentionally small: numbers, strings, booleans, and object instances. That keeps the interpreter focused on scope, references, object allocation, and expression evaluation rather than a large standard library."
      ],
      bullets: [
    ...
    },
    {
      ...
      paragraphs: [
        "The parser grammar is written in PEG form and emits plain parser nodes with token-type labels. A separate compiler pass converts those raw nodes into a typed program representation. This second pass is important because it turns a syntax-shaped tree into runtime-shaped data.",
      ...
      equations: [
        {
          ...
          caption:
            "The compiler maps parser output into an explicit typed program structure consumed by the interpreter."
        }
      ],
      bullets: [
      ...
      equations: [
        {
          ...
          caption:
            "The interpreter state combines heap, stack frames, active instance, class table, and function table."
        },
        {
          label: "Scoped lookup",
          tex: "v(a.b.c) = H\\left[\\operatorname{scope}(H[\\operatorname{scope}(a)].b).c\\right]",
          caption:
            "Member access is resolved by following instance scopes through heap pointers until the final field is found."
        }
      ]
    },
    {
      ...
      paragraphs: [
        "The portfolio integration turns the local interpreter into a browser-native playground. Monaco provides the editable source surface, a Monarch tokenizer gives JaggerScript-specific highlighting, and a diagnostics pass marks syntax problems as the user edits.",
        "The examples rail is as important as the editor. A language demo without small, runnable programs asks too much of the reader. The playground makes the interpreter concrete by loading programs that each exercise a different runtime behavior."
      ],
      bullets: [
    ...
    },
    {
      ...
      paragraphs: [
        "The most useful lesson from JaggerScript is that a small language still benefits from production-shaped boundaries. A parser, compiler, runtime, diagnostics layer, and editor integration each deserve their own ownership line.",
        "The project also shows why browser execution is valuable for language tooling. When the user can edit an example, run it immediately, and see output beside the source, the implementation becomes easier to evaluate than a README-only interpreter."
      ],
      bullets: [
